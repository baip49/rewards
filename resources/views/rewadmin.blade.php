<x-layouts.app :title="__('dashboard.rewadmin')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
                ...
                @can('isAdmin', App\Models\User::class)
                    ...
                    <dialog id="createModal" class="modal bg-white dark:bg-zinc-900 p-6 rounded-xl max-w-md w-full z-50">
                        <form method="POST" action="{{ route('rewards.store') }}" class="space-y-4">
                            ...
                            <div>
                                ...
                                <input id="createTitle" name="title" type="text" required
                                    class="mt-1 block w-full p-2 border rounded" />
                            </div>

                            <div>
                                <label for="createDescription"
                                    class="block text-sm font-medium text-white">Descripción</label>
                            ...
                            <div>
                                ...
                                <input id="createCost" name="cost" type="number" required
                                    class="mt-1 block w-full p-2 border rounded" />
                            </div>

                            <div>
                                <label for="createStock" class="block text-sm font-medium text-white">Stock</label>
                                <input id="createStock" name="stock" type="number" required
                                    class="mt-1 block w-full p-2 border rounded" />
                            </div>
                    ...
                    <div class="grid xl:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-5 mb-6 mx-6">
                        @foreach ($rewards as $reward)
                            <x-rewards.reward-card2 :id="$reward->id" :title="$reward->title" :description="$reward->description" :cost="$reward->cost"
                                :stock="$reward->stock" :points="auth()->user()->points" />
                        @endforeach
                    </div>
                @else
        ...
        <!-- Toast Component -->
        <div id="toast"
            class="hidden fixed bottom-10 right-10 px-4 py-3 rounded-lg text-white flex items-center space-x-2 shadow-lg z-50">
