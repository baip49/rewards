@props(['id', 'title', 'description', 'cost', 'stock', 'image' => "", 'points', 'type' => 'reward'])
         ...
         <div class="flex gap-2 items-center">
            @if ($image)
               <div class="mask-radial-at-center mask-radial-from-100% bg-cover bg-center"
                 style="background-image: url('{{ asset('storage/' . $image) }}'); width: 50px; height: 50px; border-radius: 50%;"></div>
            @else
...
<div id="modal-redeem-{{ $id }}"
   ...
   <div
      class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/20 p-8 rounded-xl space-y-6 shadow-lg">
...
<div id="modal-edit-{{ $id }}"
   ...
   <div
      ...
      <flux:heading size="lg">{{ __('reward-card.editing') . ' ' . $title }}</flux:heading>
